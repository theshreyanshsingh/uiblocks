...
const Chat: NextPage = () => {
  const [selected, setSelected] = useState<'palette' | 'ai'>('palette');
  const options = ['palette', 'ai'];

  return (
    <div className="flex flex-col h-screen w-[30vw] bg-[#0F0F0F] border-l border-[#201F22]">
      {/* Head */}
        ...
        <div className="relative flex bg-[#1A1A1A] rounded-lg px-1 py-1 w-40">
          ...
          {options.map((option) => (
            <button
              ...
              className="relative flex-1 text-sm font-sans font-medium text-white py-1 z-10 justify-center items-center flex"
            >
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        {selected === 'ai' ? (
          <div className="h-full" style={{ display: selected === 'ai' ? 'block' : 'none' }}>
            <Messages />
          </div>
        ) : (
          <div className="h-full" style={{ display: selected === 'palette' ? 'block' : 'none' }}>
            <Palette />
          </div>
        )}
      </div>
      ...
      {/* Input Keyboard */}
      <div className="p-3 border-t border-[#201F22] bg-[#1a1a1b]">
        <Keyboard />
      </div>
    </div>
  );
