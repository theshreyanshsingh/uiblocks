...
import { setFullscreen, setResponsivess } from '@/app/redux/reducers/projectOptions';
...
const SubHeader: NextPage = () => {
  const { fullscreen, responsive } = useSelector((state: RootState) => state.projectOptions);
  ...
  return (
    <div className="w-full p-2 flex justify-between items-center bg-[#0e0e0f] shadow-md space-x-5">
      {/* Refresh Button */}
      <button onClick={handleRefresh} className="flex items-center cursor-pointer text-sm text-white">
        <FaRegCopy />
      </button>
      <button onClick={handleRefresh} className="flex items-center cursor-pointer text-sm text-white">
        <RiRefreshLine />
      ...
      <div className="flex items-center space-x-4">
        ...
        >
          {responsive === 'mobile' ? <CiLaptop className="text-lg" /> : <CiMobile1 className="text-lg" />}
        </button>
        <button
          onClick={() => {
            if (fullscreen) {
          ...
          className="flex items-center cursor-pointer text-white text-sm"
        >
          {fullscreen ? <RiFullscreenExitLine className="text-lg" /> : <BiFullscreen className="text-lg" />}
        </button>
      </div>
    </div>
  );
